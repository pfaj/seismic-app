...
export async function populateImages(project) {
  ...
  const navigation = Array.from(slideShow.children).filter(
...
export async function populateDetails(project) {
  ...
  let client = ViewFunctions.buildTextEntry(
    "h3",
    "Client: " + project.clientName,
  );
  details.append(client);

  let category = ViewFunctions.buildTextEntry(
    "h3",
  ...
  let description = project.description
    ? ViewFunctions.buildTextEntry("p", project.description)
    : "";
